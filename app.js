...
//modal close function
  ...
  //function of clicked group 
  function clickedgroup(groupName) {
    modalDiv.innerHTML = '';
    modalOpen();
   
    var thisData = {};
    for (var key2 in data[0].groups) {
      if (groupName === data[0].groups[key2].name) {
        thisData = data[0].groups[key2];
    ...
    for (var key in thisData.standings) {
      ...
      trr.innerHTML = `<td>${thisData.standings[key].team.name}</td>
...
// to creating divs according to the groups and write the group name within div
    for (var key in data[0].groups) {
   
      mainDiv.innerHTML += `<div class='groups' onclick="clickedgroup('${data[0].groups[key].name}')">${data[0].groups[key].name}</div>`;
